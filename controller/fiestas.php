<?php

class Fiesta
{
    public function reservar($datos)
    {
        $error = "";
        $fecha = addslashes($datos['eventDate']);
        $tipo = addslashes($datos['eventType']);
        $correo = $datos['contactEmail'];
        $nombre = addslashes($datos['eventName']);
        $info = addslashes($datos['additionalInfo']);
        
        $consulta = "SELECT correoElectronico FROM contacto";
		$DB = new db();
        $resultado = $DB->leer($consulta);
        $nacional = $resultado[0];
        $emailNacional = $nacional['correoElectronico'];

        $emailSubject = 'Recibiste una solicitud de reserva de la página web de Nacional';
        $headers = ['From' => $correo, 'Reply-To' => $correo, 'Content-type' => 'text/html; charset=utf-8'];
        $bodyParagraphs = ["Correo: {$correo}", "Nombre de Evento: {$nombre}", "Fecha Deseada: {$fecha}", "Tipo de Evento: {$tipo}", "Información adicional: {$info}"];
        $body = join(PHP_EOL, $bodyParagraphs);

        if (!mail($emailNacional, $emailSubject, $body, $headers)) {
            $error = 'Ups, algo falló. Intentalo más tarde.';
        }
        return $error;
    }

}